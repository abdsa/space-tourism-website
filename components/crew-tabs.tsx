...
const CrewTabs: React.FunctionComponent<ICrewTabsProps> = (props) => {
  return (
    <>
      <amp-selector
        className="tabs-with-selector crew-tabs grid grid-flow-col place-content-center gap-10 justify-self-center font-barlowC text-bodyText tracking-navText text-white desktop:-mt-[5vh] desktop:justify-self-start "
        role="tablist"
        ...
        {jsonData.crew.map((item, index) => {
          return (
            <button
              ...
              key={`${index}`}
              className="rounded-full uppercase transition-[border-bottom] duration-150"
              tabIndex={0}
            >
              <svg
                width="15"
